...
public class MainControl : MonoBehaviour
{
    ...
    IEnumerator ieSortCubeLis()
    {
        ...
        {
            int min = i;
            for (int j = i+1; j < m_cubeLis.Count; j++)
            {
                if (m_cubeLis[j].GetComponent<GameControl>().m_slider.value <
                   m_cubeLis[min].GetComponent<GameControl>().m_slider.value)
                    min = j;
                
            }
            GameObject _tObj = m_cubeLis[i];
            m_cubeLis[i] = m_cubeLis[min];
            m_cubeLis[min] = _tObj;
            ChangePostionFor2(i, min);
